...
- **정보 탐색은 이미 완료된 상태**여야 한다 (아래 §6 "통합 정보 탐색" 참조). 탐색 결과를 기반으로 포스트를 작성한다
...
## 6. 통합 정보 탐색 (Universal Information Gathering)
...
사용자가 특정 주제에 대해 질문하면, **§6 통합 정보 탐색을 먼저 실행**한다. 블로그 포스트, 교재 소스, agent 사전지식을 **모두 동시에** 탐색한 뒤, 3개 레이어의 결과를 통합하여 답변한다.

### 탐색 절차

§6의 병렬 탐색 프로토콜을 그대로 따른다. Q&A라고 해서 교재 탐색을 생략하지 않는다.
...
2. **분할 판단** — 하나의 TBD 내용이 반드시 1개 포스트가 될 필요는 없다. 아래 기준으로 복수 포스트 분할 여부를 판단한다:
...
3. **중복 확인** — 이미 정식 포스트로 작성된 내용이 있는지 대조
4. **분류** — 적절한 카테고리 폴더에 매핑
5. **재작성 및 보강**:
...
6. **index.qmd 업데이트**
7. **TBD.qmd 초기화** — 전환 완료 후 빈 템플릿 상태로 되돌린다 (파일 삭제 금지)
...
**태스크 유형에 관계없이**, 사용자 요청을 받으면 §6의 병렬 탐색 프로토콜을 먼저 실행한다. 탐색 결과를 확보한 상태에서 아래 태스크별 절차를 진행한다.

```
사용자 요청
    ↓
§6 통합 정보 탐색 (블로그 + 교재 + 사전지식, 병렬)
    ↓
태스크 유형 판별 + 탐색 결과를 기반으로 실행
```

> **예외**: GUIDE.md 자체 수정처럼 콘텐츠 탐색이 불필요한 태스크는 §6을 생략할 수 있다.

### 태스크별 절차

| 태스크 유형 | 로드할 가이드 | 핵심 절차 |
|---|---|---|
| 새 블로그 포스트 작성 | AGENT_GUIDE + 해당 Category GUIDE | §6 탐색 → Step 1~6 체크리스트 |
| 기존 포스트 보강/수정 | AGENT_GUIDE + 해당 Category GUIDE | §6 탐색 → 대상 파일 읽기 → 수정 → index.qmd 확인 |
| 주제에 대한 질문 답변 | AGENT_GUIDE §7 (질문 응답 가이드) | §6 탐색 → 3개 레이어 통합 답변 |
| 시리즈 정리/재구성 | AGENT_GUIDE §8 (시리즈 정리) + Category GUIDE | §6 탐색 → 전체 읽기 → 진단 → 설계 → 파일 처리 |
| TBD.qmd 전환 | AGENT_GUIDE §8 (임시 메모 전환) + 대상 Category GUIDE | §6 탐색 → 분류 → 재작성 → index 업데이트 → 초기화 |
| GUIDE.md 자체 수정 | AGENT_GUIDE (이 파일) | 기존 규칙 보존, 추가/구조화만 (§6 생략 가능) |
