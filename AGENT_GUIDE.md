...
2. **관련 챕터 특정**: 질문/주제와 관련된 챕터를 키워드 매칭으로 찾는다
3. **Full MD에서 상세 확인**: Summary의 `source_file`과 `line` 정보를 따라 원본의 해당 챕터로 직접 이동하여 수식, 정의, 증명, 예시 등 상세 내용을 읽는다
4. **블로그 스타일로 재작성**: 원본 내용을 그대로 복붙하지 않고, 한다 체로 재작성하며 `(저자, 연도, Ch.N)` 형식으로 인용한다
...
- Full MD를 전부 읽지 않는다 — Summary에서 특정한 챕터만 선택적으로 읽는다
