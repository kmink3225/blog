...
### Step 1-5: 기존 포스트 중복 확인 (필수)

새 파일을 만들기 전에 **반드시** 기존 포스트와의 내용 중복을 확인한다.

**확인 방법**:
- 해당 카테고리 폴더의 파일 목록을 확인한다
- 관련 카테고리의 `index.qmd`에서 비슷한 주제의 링크가 있는지 확인한다
- 핵심 키워드로 `Grep`을 사용해 기존 파일을 검색한다
...
| 기존 파일이 있지만 다루는 **관점·깊이·범위가 다름** | 기존 파일에 섹션을 추가하거나, 별도 파일로 분리한다 |
