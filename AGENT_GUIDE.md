...
- **이모지 사용 금지**
- **섹션 헤더에 수동 번호 금지**: `## 1. 개요`, `### 2.1 정의` 처럼 직접 번호를 붙이지 않는다. `_quarto.yml`에서 `number-sections: true`가 전역 설정되어 있으므로, 렌더링 시 heading depth에 따라 번호가 자동 부여된다. 수동 번호를 붙이면 `1. 1. 개요`처럼 이중 번호가 생긴다.
  - O: `## 개요`
  - X: `## 1. 개요`
...
### Step 1.5: 카테고리 GUIDE.md 전체 읽기 (필수)

해당 카테고리 폴더의 `GUIDE.md`를 **전체** 읽는다. GUIDE.md에는 카테고리별 콘텐츠 구조, 용어 규칙, 참조 교재, 관련 주제 링크 패턴, 예시 소재 등이 정의되어 있다. 이 파일을 읽지 않고 포스트를 작성하면 카테고리의 기존 관습과 어긋나는 결과물이 나온다.

**읽기 범위**: 부분 읽기가 아니라 **전체를 읽는다**. GUIDE.md는 카테고리당 하나뿐이고, 대부분 200줄 이내이므로 토큰 비용이 작다. 구조, 용어, 교재, 크로스 링크 규칙이 파일 전체에 흩어져 있으므로 부분 읽기로는 규칙을 놓칠 수 있다.

**GUIDE.md가 없는 카테고리**: 이 단계를 건너뛴다. 단, YAML frontmatter의 `category_guides` 목록에서 해당 카테고리에 GUIDE가 등록되어 있는지 먼저 확인한다.

**Multi-disciplinary 주제**: 주 카테고리의 GUIDE.md를 읽은 후, 관련 카테고리의 GUIDE.md도 함께 읽는다 (위 "Multi-disciplinary 주제 작성 원칙" 테이블 참조).

### Step 1.7: 기존 콘텐츠 중복 판단 (필수)

새 파일을 만들기 전에 **반드시** 기존 포스트와의 내용 중복을 확인한다. 같은 카테고리뿐 아니라 관련 카테고리도 검색한다.
...
4. 기존 파일의 YAML 헤더(title, description)를 읽어 범위를 파악한다
5. 필요 시 본문을 읽어 깊이와 완성도를 판단한다

**판단 기준**:

| 상황 | 조치 |
|---|---|
| 동일 주제를 다루는 파일이 이미 존재하고 내용이 **상당 부분 겹침** | 새 파일 생성 금지 — 기존 파일을 보강한다 |
...
| **Full MD** | 원본 파일명 `.md` | PDF→MD 변환 원본. 챕터 단위로 상세 내용 참조 시 사용 |

**교재 참조 절차:**

1. **Summary 먼저 읽기**: 해당 분야의 `*-summary.md`를 읽어 Contents(목차)와 챕터별 키워드를 확인한다
2. **관련 챕터/섹션 특정**: 질문/주제와 관련된 챕터를 키워드 매칭으로 찾는다. Summary에 섹션별 line 번호가 있으면 해당 섹션까지 좁힌다
3. **읽기 깊이 판단**: 아래 기준에 따라 Full MD를 얼마나 읽을지 결정한다
4. **Full MD에서 상세 확인**: 판단 결과에 따라 선택적 또는 전체 읽기를 수행한다
5. **블로그 스타일로 재작성**: 원본 내용을 그대로 복붙하지 않고, 한다 체로 재작성하며 `(저자, 연도, Ch.N)` 형식으로 인용한다
...
Full MD를 얼마나 읽을지는 **교재의 희소성**과 **주제의 표준성**으로 결정한다. 목적은 토큰 비용을 절약하면서도 사전학습 지식의 환각 위험을 방지하는 것이다.

| 교재 유형 | 주제 유형 | 읽기 전략 | 이유 |
|---|---|---|---|
| 표준 교재 (Casella & Berger, Strang 등) | 잘 알려진 정리/개념 (MLE, CLT, SVD 등) | **선택적 읽기** — Summary 키워드로 특정한 정의/정리/예시 구간만 읽기 (각 100~200줄) | 사전학습 커버리지가 높아 전체를 읽는 것은 토큰 낭비 |
| 표준 교재 | 덜 알려진 세부 주제 (특정 증명, niche 예시) | **확장 읽기** — 관련 섹션 전체 (300~500줄) | 세부 전개는 사전학습이 부정확할 수 있음 |
| Niche 교재 (Hedeker & Gibbons, Kokoszka 등) | 모든 주제 | **챕터 전체 읽기** | 사전학습 커버리지가 얇아 원문 의존도가 높음 |
| 한국어 교재 또는 미공개 자료 | 모든 주제 | **챕터 전체 읽기** | 사전학습에 거의 없을 가능성이 높음 |
