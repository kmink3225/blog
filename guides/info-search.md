...
사용자 요청 수신
    ↓
의도 분석 및 키워드 확장 (Intent-based Keyword Expansion) — **핵심 단계**
    - 질문의 표면적 단어뿐만 아니라 에이전트 지식을 활용해 연관 키워드 도출
    - 동의어, 상위/하위 개념, 관련 전문 용어 리스트업 (예: "가설검정" → "귀무가설, p-value, 유의수준")
    - 검색 범위를 넓히기 위한 복수 키워드 조합 생성
    ↓
┌─────────────────────────────────────────────────────────┐
│  동시 탐색 — 확장된 키워드 리스트로 3개 레이어를 모두 실행한다  │
│                                                         │
│  Layer 1: 블로그 포스트                                   │
...
...
### 교재 파일 부재 시 처리 (Book-Absent Guard)

`docs/book/` 관련 파일이 없거나 찾을 수 없는 경우:

| 상황 | 행동 |
|------|------|
| Summary MD 없음 | Full MD 직접 탐색. 그것도 없으면 아래 규칙 적용 |
| Full MD 없음 | agent 사전학습 기반으로 서술 — **반드시 `(교재 미확인 — agent 사전학습 기반)` 표기** |
| 교재 폴더 자체 없음 | AGENT_GUIDE_CORE.md Fallback 테이블 적용. 포스트 작성 후 미참조 명시 |

**절대 금지**: 교재를 확인하지 않은 내용을 "교재에서 읽은 것처럼" 서술하지 않는다. 수식·정리 번호(예: "식 3.4.1")를 날조하지 않는다. 읽지 않은 챕터 내용을 원문 기반인 것처럼 인용하지 않는다.

```
# WRONG: 교재 미확인 상태에서 원문 번호 인용
"Casella & Berger (2002, 식 3.3.22)에 따르면 ..."  ← 파일을 읽지 않았으면 금지

# CORRECT: 미확인 명시
"라플라스 분포의 PDF는 다음과 같다 (교재 미확인 — agent 사전학습 기반):"
```
