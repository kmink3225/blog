...
- 다음 번호를 결정할 때 **서브 인덱스 규칙**을 따른다:
  - 기본: 기존 최대 번호 + 1 (예: 138 다음은 139)
  - **번호가 선점된 경우**: 한 칸씩 밀지 않고 **서브 인덱스를 확장**한다 (예: 95번이 있으면 `95-1`, `95-2`). 이는 기존 크로스 링크를 깨뜨리지 않기 위함이다
  - 리팩토링으로 파일을 이동할 때도 동일한 규칙 적용: 연관 번호대 근처에 서브 인덱스로 배치
...
기존 포스트와 주제가 겹친다고 판단한 경우, **기존 포스트 전체를 읽고** 요청 내용과 정밀 비교한다.
"중복"처럼 보여도 고유한 관점, 날카로운 프레이밍, 구체적 예시가 남아있을 수 있다.
그 고유 내용을 기존 포스트에 통합하는 것이 새 포스트를 만드는 것보다 **우선**이다.
:::
