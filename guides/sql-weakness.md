...
| 패턴 | 최근 발생 | 누적 횟수 | 비고 |
|------|---------|---------|------|
| 경계값 혼동 (`>` vs `>=`) | 2026-04-07 | 2 | "이후" = `>=` 임을 매번 확인 필요. 2026-04-07 세션: 1차 통과 1회, 힌트 후 수정 1회 → 연속 카운트 리셋. 2026-04-08: 1차 통과 (연속 1회) |
| 문자열 따옴표 (큰따옴표 사용) | 2026-04-07 | 1 | MySQL은 허용하나 표준 SQL은 작은따옴표. 2026-04-07: IFNULL 문제 1차 통과 (연속 +1) |
| ORDER BY 다중 정렬 문법 | 2026-04-07 | 2 | `AND` 사용 → 쉼표(`,`)로 구분해야 함. 2026-04-07: 1차 통과 1회 후 AND 오류 재발 → 연속 카운트 리셋, 누적 2. 2026-04-08: 쉼표 구분 1차 통과 (연속 1회) |
...
| SQL 키워드 오타 | 2026-04-08 | 1 | `ordered by` → `ORDER BY`. 키워드 철자 오타로 실행 불가. 힌트 후 수정 |
