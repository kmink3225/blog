...
type: category
version: 1.0
description: LOAD when writing posts about A/B testing, causal inference, MAB, experiment design, or epidemiology. Covers experimental methodology from design through analysis with cross-disciplinary connections to Statistics and Engineering.
...
- **교재 기반**: `docs/book/epidemiology/` (Hernan, Woodward, Schulz 등)을 논리적 뼈대로 활용하되, agent의 최신 지식으로 보완한다
...
- 추상적이거나 이해하기 어려운 개념에는 비유, 반사실적 사고, 역사적 맥락 등 직관적 설명을 **수식/정의가 등장하는 바로 그 위치에** 포함한다. 별도 섹션으로 분리하지 않고, 추상 개념과 직관을 **붙여 놓는다**. 특히 다음에는 반드시 직관을 붙인다:
  - 통계적 가정(SUTVA, ignorability, positivity): "이 가정이 깨지면?" 반사실 시나리오
  - 보정 방법(Bonferroni, Alpha Spending): "보정 안 하면?" + 보정의 비유
  - 수식이 처음 등장할 때: 수식이 뜻하는 바를 일상어 1~2문장
  - 지표 점수(κ, Cohen's d, ICC): 해당 값이 실무에서 체감되는 차이로 번역
...
<fix-stats-without-design>

- **WRONG**: 통계적 검정 결과만 보고 (p-value, 신뢰구간만 제시하고 실험 설계 과정 생략)
- **CORRECT**: 실험 설계(randomization, sample size) → 검정 → 해석 순서를 반드시 따른다. 검정 결과의 타당성은 설계 품질에 의존하므로, 설계 단계(무작위 배정 방법, 표본 크기 산출 근거, 검정력 분석)를 먼저 서술한 후 검정 결과를 제시한다.

</fix-stats-without-design>

<fix-missing-assumption>

- **WRONG**: A/B 테스트 결과 해석만 제시 (처치군 vs 대조군 차이가 유의하다고만 결론)
- **CORRECT**: SUTVA(Stable Unit Treatment Value Assumption), 독립성, positivity 등 핵심 가정을 먼저 명시하고, 각 가정의 위반 가능성(네트워크 효과, spillover, 자기선택 등)을 점검한 뒤 결과를 해석한다. 가정이 위반되면 결과 해석이 어떻게 달라지는지 반드시 논의한다.
...
- 실험 설계 원리 설명 (randomization, blocking, stratification, sample size 계산 등)
- 인과추론 프레임워크 적용 (Potential Outcomes, DAG, do-calculus 등)
- 역학 ↔ 통계 ↔ IT 3열 매핑 (용어, 지표, 방법론의 cross-domain 대응)
- 교재 기반 서술 (`docs/book/epidemiology/` 등 명시된 교재를 논리적 뼈대로 활용)

**CANNOT (이 가이드가 금지하는 것)**:

- 인과관계를 상관관계로 혼동하는 서술 (관찰적 연관을 인과적 효과로 표현하는 것을 금지)
- 실험 가정을 생략한 채 결론 제시 (SUTVA, ignorability, positivity 등 핵심 가정을 명시하지 않고 처치효과를 주장하는 것을 금지)
