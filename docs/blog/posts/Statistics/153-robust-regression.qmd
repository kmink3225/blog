...
Casella & Berger(2002) Ch.12 회귀 모형의 세 번째 주제 §12.4 — 강건 회귀(Robust Regression)이다. 앞서 [로지스틱 회귀](./glm/02-logistic-regression-model.qmd)가 반응변수의 분포를 바꾸었다면, 이 포스트는 **적합 기준 자체를 바꾼다** — 제곱합 손실을 강건한 손실함수로 교체한다.
...
- [로지스틱 회귀: 추정](./glm/03-logistic-regression-estimation.qmd) — IRLS의 GLM 버전
