...
증명의 핵심 아이디어는 놀랍도록 간단하다. $E[g(X)] = \int g(x) f(x) dx \geq \int_{\{g(x) \geq r\}} g(x) f(x) dx \geq r \cdot P(g(X) \geq r)$ 이다. 적분 영역을 줄이면 값이 줄고, $g(x) \geq r$ 인 영역에서 $g(x)$ 를 $r$ 로 대체하면 더 줄어든다.
