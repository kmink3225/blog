---
title: "조건부분포와 독립성 (Conditional Distributions and Independence)"
subtitle: "조건부 PMF/PDF, 조건부 기댓값, 조건부 분산, 독립성의 정의와 판별, MGF의 곱 정리"
description: |
  결합분포��서 한 변수의 값을 관측하면 다른 변수의 불확실성이 어떻게 변하는가?
  조건부분포는 이 질문에 답한다. 독립성은 조건부분포가 주변분포와 같아지는 특별한 경우이다.
  이 포스트에서는 조건부 PMF/PDF의 정의, 조건부 기댓값과 분산, 독립성의 정의와 판별법,
  그리고 독립 확률변수의 MGF 곱 정리를 교재 기반으로 다룬다.
categories:
  - Statistics
author: Kwangmin Kim
date: 03/30/2026
...
[결합분포와 주변분포](./80-joint-marginal-distributions.qmd)에서 결합분포가 두 변수의 동시적 확률 구조를 완전히 기술한다는 것을 보았다. 그러나 실무에서는 종종 **한 변수의 값을 관측한 상태**에서 다른 변수에 대해 추론해야 한다.

**실무적 질문 예시**:
- 키가 73인치인 사람의 체중은 어떻게 분포하는가?
- 약물 투여량이 50mg일 때 반응 변수의 기대값은 얼마인가?
- 오늘 비가 온다는 정보가 있을 때, 내일 비가 올 확률은 얼마인가?
...
::: {.callout-note}
## 정의: 조건부 PMF (Casella & Berger, 2002, Definition 4.2.1)
...
::: {.callout-note}
## 정의: 조건부 PDF (Casella & Berger, 2002, Definition 4.2.3)
...
핵심: $E[Y|X]$ 는 $X$ 의 **함수**이며, 따라서 그 자체가 **확률변수**이다. $X = x$ 일 때 $E[Y|X]$ 의 값은 $E[Y|x]$ 이다.
...
비교: $Y$ 의 주변분포�� 감마 $\text{Gamma}(2, 1)$ 로 $\text{Var}(Y) = 2$ 이다. $X = x$ 를 관측하면 분산이 2에서 1로 줄어든다. **조건부 정보가 불확실성을 감소시킨다**는 직관이 여기에서 확인된다.
...
::: {.callout-note}
## 정의: 독립 확률변수 (Casella & Berger, 2002, Definition 4.2.5)

이변량 확률벡터 $(X, Y)$ 에서 $X$ 와 $Y$ 가 **독립**이란, 모든 $x \in \mathbb{R}$ , $y \in \mathbb{R}$ 에 대해:
...
::: {.callout-note}
## 보조정�� 4.2.7 (Casella & Berger, 2002, Ch.4)
...
1. 양수 영역이 직적인가? (아니면 즉시 비독립)
2. 결합 PDF/PMF가 $g(x) h(y)$ 로 분리되는가? (보조정리 4.2.7)
3. 위 두 조건이 모두 만족되면 독립
:::
...
::: {.callout-note}
## 정리 4.2.10 (Casella & Berger, 2002, Ch.4)

$X$ 와 $Y$ 가 독립이면:

(a) 모든 $A, B \subset \mathbb{R}$ 에 대해 $P(X \in A, Y \in B) = P(X \in A) \, P(Y \in B)$

(b) $g(x)$ , $h(y)$ 가 각각 $x$ , $y$ 만의 함수이면 $E[g(X) h(Y)] = E[g(X)] \cdot E[h(Y)]$
:::
...
��라서 $\text{Cov}(X, Y) = E[XY] - E[X]E[Y] = 0$ 이다. **독립이면 무상관**이다. 그러나 [다변량 확률변수 개요](./79-multiple-random-variables-overview.qmd)에서 언급했듯이, 역은 성립하지 않는다.

### 적률생성함수의 곱

::: {.callout-note}
## 정리 4.2.12 (Casella & Berger, 2002, Ch.4)
...
::: {.callout-note}
## 정리 4.2.14 (Casella & Berger, 2002, Ch.4)
...
| 신뢰구간 | 조건부 분산 | 예측 구간: $\text{Var}(Y|X) $ 이 구간 폭 결정 |
...
- [결합분포와 주변분포](./80-joint-marginal-distributions.qmd) -- 결합/주변분포의 정의
- [다변량 확률변수 개요](./79-multiple-random-variables-overview.qmd) -- Ch.4 전체 조감

**후속 주제**

- [이변량 변환](./bivariate-transformations.qmd) -- 야��비안 방법
- [계층모형과 혼합분포](./hierarchical-models-mixtures.qmd) -- 전체 기대값의 법칙
- [공분산과 상관계수](./covariance-correlation.qmd) -- 독립/무상관의 정량적 구분

**관련 개념**

- [적률생성함수](./53-moments-mgf.qmd) -- MGF의 정의와 성질
- [정규 분포](./68-normal-distribution.qmd) -- 독립 정규 합의 정규성
- [확률 부등식과 항등식](./78-inequalities-identities.qmd) -- 코시-슈바르츠로 상관계수 바운드
