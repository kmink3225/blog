...
[데이터 축소의 원리 overview](./146-data-reduction.qmd)에서 우도원리를 전체 데이터 축소 원리의 맥락에서 간략히 다루었다. 이 포스트에서는 **우도원리(The Likelihood Principle)** 를 세분화하여, 우도함수의 본질적 의미, 형식적 정의, 증거함수(evidence function) 개념, 비른바움 정리의 증명 구조, 빈도주의 통계학과의 충돌, 그리고 실무적 논쟁까지 깊이 있게 다룬다 (Casella & Berger, 2002, Ch.6.3).
...
- [등변원리](./146-data-reduction.qmd#등변원리-the-equivariance-principle) — 현재 overview에 포함, 세분화 예정
- [점추정: 최대우도추정법](./101-maximum-likelihood-estimators.qmd) — 우도함수를 최대화하는 추정 방법
- [가설검정과 우도비 검정](./146-data-reduction.qmd) — 우도비를 검정에 활용
