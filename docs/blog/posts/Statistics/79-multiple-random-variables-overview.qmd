...
결합분포에서 주변분포를 유도하는 것은 항상 가능하다. 그러나 **주변분포만으로 결합분포를 복원하는 것은 일반적으로 불가능**하다. 같은 주변분포를 가지면서 서로 다른 결합분포가 무한히 존재할 수 있다. 결합분포는 주변분포보다 더 많은 정보를 담고 있으며, 적분/합산 과정에서 변수 간 관계 정보가 소실된다.
...
독립이면 반드시 무상관( $\text{Cov}(X, Y) = 0$ )이다. 그러나 **무상관이라고 독립은 아니다**. 예를 들어 $X \sim N(0, 1)$ 이고 $Y = X^2$ 이면 $\text{Cov}(X, Y) = E[X^3] = 0$ 이지만, $Y$ 는 $X$ 에 의해 완전히 결정되므로 독립이 아니다. 무상관은 선형 관계의 부재만 의미하고, 독립은 모든 형태의 관계가 부재함을 의미한다.
