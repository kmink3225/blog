...
```python
# 단일 모수 가족: lambda = 1 로지스틱, lambda -> 0 cloglog
...
def link_family_logit_cloglog(pi, lam):
    """lam=1: logit, lam->0: cloglog 한계."""
    if abs(lam) < 1e-8:
        return np.log(-np.log(1 - pi + 1e-12))
    return np.log(((1 / (1 - pi + 1e-12)) ** lam - 1) / lam)

# lambda 격자에서 profile deviance 를 스캔해 최적 링크 검출
# (실제 구현은 custom link 로 binomial GLM 을 적합)
```

이 코드는 cloglog 로 생성한 데이터에서 $\hat\lambda \approx 0$ 을 회수하는 것이 목표.
