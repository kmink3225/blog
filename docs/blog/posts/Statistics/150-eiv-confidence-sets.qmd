...
즉, 이 수정안은 "보장은 없지만 실용 조건에서 합리적으로 작동하는" 타협안이다.
...
정확한 신뢰 집합을 구성하려면 **피봇 통계량(pivotal quantity)**이 필요하다. Creasy(1956)와 Williams(1959)는 EIV 모형의 다음 성질에서 피봇을 발견하였다.
...
$\hat{\beta}$ 를 분자와 분모 각각 정규 분포를 따르는 두 통계량의 비로 쓸 수 있고, 이 비의 신뢰 집합이 Fieller 구간과 같은 수학적 구조를 가진다.
