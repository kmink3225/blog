---
title: "위치-척도족 (Location-Scale Family)"
subtitle: "표준화, 피벗, 검정 통계량의 수학적 기반 — 위치족, 척도족, 위치-척도족의 정의와 응용"
description: |
  위치-척도족은 하나의 표준 PDF로부터 위치와 척도 변환을 통해 분포 가족을 생성하는 구조이다.
  z-score 표준화, t-통계량, 배치 정규화 등 실무에서 흔히 사용하는 기법이 모두 이 구조에 기반한다.
  이 포스트에서는 위치족, 척도족, 위치-척도족의 정의를 교재 기반으로 엄밀히 다루고,
  표준화 정리, 피벗 양, 그리고 데이터 사이언스 응용까지 코드와 함께 살펴본다.
categories:
  - Statistics
author: Kwangmin Kim
date: 03/30/2026
...
위치 모수 $\mu$ 는 PDF의 그래프를 수평으로 이동시킨다. 그래프의 **모양(shape)**은 전혀 변하지 않고, $x = 0$ 위에 있던 점이 $x = \mu$ 위로 옮겨진다.
...
$X$ 가 PDF $\frac{1}{\sigma} f\!\left(\frac{x - \mu}{\sigma}\right)$ 를 가지는 것은, PDF $f(z)$ 를 가지는 확률변수 $Z$ 가 존재하여 $X = \sigma Z + \mu$ 인 것과 **동치**이다.
:::

이 정리가 핵심인 이유: $X$ 를 관측한 뒤 $Z = (X - \mu)/\sigma$ 로 변환하면, $Z$ 의 분포는 표준 PDF $f(z)$ — 즉 **모수 $\mu$ , $\sigma$ 에 의존하지 않는다**. 이것이 표준화(standardization)의 수학적 근거이다.
...
### 피벗 양 (Pivotal Quantity)

위치-척도 구조에서 자연스럽게 **피벗 양**이 등장한다 (Casella & Berger, 2002, Example 9.2.7). 피벗 양이란 데이터와 모수의 함수이면서 그 분포가 모수에 의존하지 않는 양이다.
...
정규분포에서 $(\bar{X} - \mu)/(S/\sqrt{n})$ 이 $t_{n-1}$ 분포를 따르는 것은, 이 양이 위치-척도 피벗이기 때문이다. 피벗 양은 **신뢰구간 구성**의 핵심 도구이다.

### 모수 해석의 보편성

위치-척도족에서 $\mu$ 와 $\sigma$ 의 해석은 **분포에 관계없이 동일**하다:
...
핵심: $\mu = 3$ , $\sigma = 2$ 라는 특정 모수에 대한 확률을 **표준 분포( $\mu = 0$ , $\sigma = 1$ )에서의 계산으로 환원**하였다.
...
위치-척도족과 [지수족(Exponential Family)](./76-exponential-family.qmd)은 **독립적인 분류 기준**이다. 두 기준이 교차하는 영역을 정리한다.
...
- [정규 분포](./68-normal-distribution.qmd) — 위치-척도족의 대표적 예시
- [구간 추정](./interval_estimation.qmd) — 피벗 양 기반 신뢰구간
- [강건 통계](./robust_estimation.qmd) — 위치-척도 추정의 강건 버전

**관련 개념**

- [지수족 (Exponential Family)](./76-exponential-family.qmd) — 독립적 분류 기준, 교차 관계
- [분포 가족 개요](./55-common-families-overview.qmd) — 위치-척도족 요약
- [코시 분포](./73-common-families-continuous-cauchy-distribution.qmd) — 위치-척도족이지만 지수족이 아닌 예시
- [라플라스 분포](./75-common-families-continuous-laplace-distribution.qmd) — 위치-척도족 + 강건 통계 응용
