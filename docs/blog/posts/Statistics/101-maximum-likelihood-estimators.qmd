...
#### $E[\hat{\sigma}^2_{\text{MLE}}]$ 의 유도

$$
\begin{aligned}
E\!\left[\frac{1}{n}\sum_{i=1}^n (X_i - \bar{X})^2\right]
&= \frac{1}{n}\sum_{i=1}^n E\!\left[X_i^2 - 2X_i\bar{X} + \bar{X}^2\right] \\
&= \frac{1}{n}\left(\sum_{i=1}^n E[X_i^2] - 2\sum_{i=1}^n E[X_i\bar{X}] + nE[\bar{X}^2]\right)
\end{aligned}
$$
...
&= n(\mu^2 + \sigma^2) - 2n\!\left(\mu^2 + \frac{\sigma^2}{n}\right) + n\!\left(\mu^2 + \frac{\sigma^2}{n}\right) \\
...
이것은 **OLS(최소제곱) 추정량과 정확히 동일**하다. 정규 오차 가정 하에서 MLE = OLS 이다.
...
**실무적 의미**: 회귀 분석에서 `summary(lm(...))` 의 잔차 표준오차는 $\sqrt{S^2}$ (비편향)이고, MLE 기반 소프트웨어(예: `statsmodels` 의 `loglike`)는 $\hat{\sigma}^2_{\text{MLE}}$ 를 사용한다. 혼합 모형에서는 이 차이가 더 커지며, 이를 보정하는 것이 REML(Restricted MLE)이다 — REML에 대해서는 [LMM 추정과 모델 선택](./LDA/03-mixed-model-estimation.qmd)에서 상세히 다룬다.
...
- [Maximum Likelihood Estimation & Statistical Bias](./101-maximum-likelihood-estimators.qmd) — MLE 기초와 OLS와의 연결 (영문)
- [BLUE (Best Linear Unbiased Estimator)](./99-1-blue.qmd) — MSE와 편향-분산 트레이드오프
