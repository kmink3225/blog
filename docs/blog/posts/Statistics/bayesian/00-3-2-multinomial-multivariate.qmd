...
[§ 3.1~3.3 심화](./00-3-1-normal-marginalization.qmd) 가 단변량 정규의 공동 사후를 완전히 풀어냈다면, 이 포스트는 **같은 문법이 이산 다범주 (다항) 와 벡터 관측 (다변량 정규)** 에서 어떻게 작동하는지 심화한다. Part IV 회귀 모형의 수학적 뼈대가 여기 서 있다.
...
$\theta_1 - \theta_2$ 같은 차이, $\theta_1 / \theta_2$ 같은 비, "Bush 지지율 > 50%" 같은 확률 — **사후 시뮬 하나로 모든 파생량의 사후 분포** 를 얻는다. [§ 1.9](./00-1-3-computation-applied-exercises.qmd) 의 $S \times k$ 표 전략이 여기서 결정적 위력을 발휘.
...
- [Ch.3 개요](./00-3-0-multiparameter.qmd) — Ch.3 전체 훑기
