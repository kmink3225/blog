...
[Ch.1 Probability and Inference 개요](./00-1-0-probability-inference.qmd) 가 Ch.1 전체 (§ 1.1~1.9) 를 한 호흡으로 훑는 포스트였다면, **이 포스트는 § 1.1~1.4 를 교재 원문 수준으로 풀어 쓴** 심화편이다. 같은 4 절을 다시 다루는 이유는 Gelman 이 이 네 절에 **베이즈 데이터 분석의 언어 전체** 를 숨겨놓았기 때문이다.
...
- [Ch.1 Probability and Inference 개요](./00-1-0-probability-inference.qmd) — § 1.1~1.9 를 한 호흡으로 훑는 overview
