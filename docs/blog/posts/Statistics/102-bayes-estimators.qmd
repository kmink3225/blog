...
사후 일관성이 성립하기 위한 주요 조건: (1) 사전분포 $\pi(\theta)$ 가 참값 $\theta_0$ 의 근방에 **양의 확률을 부여**해야 한다 ($\pi(\theta_0 \text{ 근방}) > 0$), (2) 모형이 올바르게 설정되어 있어야 한다. 조건을 만족하면 $n \to \infty$ 에서 사후분포가 $\theta_0$ 에 집중된다 (Doob의 정리 또는 Schwartz의 정리). 반면, 사전분포가 참값 근방에서 0이면 아무리 데이터가 많아도 사후분포가 참값에 도달하지 못할 수 있다.
