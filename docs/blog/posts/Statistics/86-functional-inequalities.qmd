...
기하학적 직관: 볼록 함수의 그래프는 **임의의 두 점을 잇는 선분 아래에** 놓인다 (그릇 모양). 오목 함수는 **선분 위에** 놓인다 (언덕 모양).

두 번 미분 가능한 함수에 대해: $g''(x) \geq 0$ 이면 볼록, $g''(x) \leq 0$ 이면 오목.
...
볼록 함수에 확률변수를 넣으면, "기대값의 함수값"보다 "함수값의 기대값"이 더 크다. 이는 볼록 함수가 극단값을 "증폭"시키기 때문이다. $X$ 가 $E[X]$ 에서 양쪽으로 퍼져 있으면, 볼록 함수가 이 퍼짐을 양의 방향으로 더 크게 키운다.
