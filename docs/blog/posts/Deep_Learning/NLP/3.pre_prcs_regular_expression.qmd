---
title: "정규표현식 완전 정복"
subtitle: "NLP 텍스트 전처리와 Python re 모듈 실전 가이드"
description: |
  자연어 처리(NLP)에서 텍스트 전처리와 패턴 매칭에 필수적인 정규표현식의 이론과 실전을 다룬다.
  메타 문자(^, $, [], .), 반복자(*, +, ?, {m,n}), Greedy vs Lazy, 그룹화와 backreference,
  특수 문자(\w, \d, \s, \b), re 탐색 함수 4가지, Flags, re.compile과 Match 객체,
  그리고 NLP 활용 예시(URL·이메일·개체명 추출, 토큰화, 마스킹, 주민번호 변환, HTML 태그 제거 등)를 완전히 해부한다.
categories:
  - NLP
  - Engineering
  - Python
...
- **실생활 활용 예시**: 소설 이름 등장횟수, 전화번호 추출, 이메일 검증, 문맥 파악, 주민번호 변환, 로그 알람
- **메타 문자**:
    - 앵커: `^`, `$`
    - 문자 클래스: `[]`, `[^]`
    - 논리 연산자: `.`, `|`
...
- **실전 활용 사례**: 핸드폰 번호, CSV 필드, 개인정보 마스킹, 주민번호 변환, HTML 태그 제거, URL·해시태그 추출, 문장 토큰화, 개체명 추출, 단어 빈도 분석
...
일반 그룹 `(...)`은 패턴을 그룹화하고 매칭된 부분을 메모리에 저장(캡처)한다.
반면 `(?:...)`는 패턴을 그룹화하지만 매칭된 부분을 메모리에 저장하지 않는다.
단순히 패턴을 묶어서 처리하고 싶을 때 사용하며, 나중에 참조할 필요가 없을 때 메모리 효율을 위해 사용한다.

```python
import re

# 비캡처 그룹: 'ab'가 반복되는 패턴을 묶되 캡처는 하지 않음
pattern = r'(?:ab)+'
...
| 구분 | 패턴 | 캡처 여부 | 매칭 예시 |
|------|------|-----------|-----------|
| 비캡처 그룹 | `(?:ab)+` | 캡처 없음 | "ab", "abab", "ababab" |
| 캡처 그룹 + 추가 조건 | `(ab)+c` | 마지막 "ab" 캡처 | "abc", "ababc", "abababc" |
...
- `(\w+) \1` — 첫 번째 캡처 그룹(단어)이 그대로 반복되는 패턴 ("hello hello", "python python")
- `(\w+) (\w+) \2` — 두 번째 단어가 뒤에서 반복 ("hello world world")
- `(\w+) (\w+) (\w+) \3` — 세 번째 단어가 뒤에서 반복 ("apple banana cherry cherry")
...
# \b: 단어 경계 — "category"의 "cat"은 매칭되지 않음
print(re.findall(r'\bcat\b', "The cat sat in category"))  # ['cat']
# \B: 단어 경계가 아닌 위치
print(re.findall(r'\Bcat\B', "location category cat"))  # ['cat'] — location의 중간 'cat'
```
...
## 순서 바꾸기 — re.sub와 backreference의 결합
...
정규표현식은 자연어 처리(NLP)에서 다양한 텍스트 전처리 및 정보 추출 작업에 활용된다.
복잡한 언어 패턴 분석에는 한계가 있어 고급 NLP 작업에는 딥러닝 모델과 함께 사용되는 경우가 많지만,
간단하고 명확한 패턴 기반 처리에는 매우 유용하다.
...
print(urls)
# ['https://example.com/page', 'http://test.co.kr', 'www.resources.org/docs']
```
...
print(sentences)
# ['안녕하세요', '정규표현식을 배우고 있습니다', '어렵지만 유용하죠', '화이팅...']
```
...
간단한 규칙 기반으로 정확한 결과를 보장하기는 어렵지만,
규칙을 정밀하게 보완하면 어느 정도 후보군을 추릴 수 있다.
더 정확한 개체명 인식은 NER(Named Entity Recognition) 모델과 함께 사용하는 것을 권장한다.
...
| **그룹화** | `()` (캡처 그룹), `(?:...)` (비캡처 그룹), `(?P<name>)` (named group), `\n` (backreference) |
...
NLP 파이프라인의 전처리 단계에서 텍스트 정제, 토큰화, 정보 추출에 매우 유용하며,
