...
* 양방향 LSTM 기반
* 문맥화된 표현: $\text{ELMo}_k^{task} = \gamma^{task} \sum_{j=0}^L s_j^{task} \mathbf{h}_{k,j}^{LM}$
* 여기서 $s_j^{task}$는 학습 가능한 가중치, $\mathbf{h}_{k,j}^{LM}$는 각 레이어의 hidden state
* 핵심 아이디어
   * **"언어 모델의 내부 표현을 임베딩으로 사용하자"**
* 기본 구조

```
입력 문장: "The bank was closed"

Forward LSTM:  [The] → [bank] → [was] → [closed]
Backward LSTM: [closed] ← [was] ← [bank] ← [The]
```
