---
title: "LangGraph 소개"
subtitle: "LangGraph 핵심 문법과 상태 기반 워크플로우 소개"
description: |
    LangGraph에서 자주 사용하는 Python 문법(`TypedDict`, `Annotated`, `add_messages`)과
    상태(state) 설계 원칙을 설명한다. TypedDict의 사용 이유, Annotated의 메타데이터 활용,
    그리고 LangGraph의 상태 관리와 메시지 리듀서 패턴을 예제로 제시한다.
categories:
    - Agent
    - LangGraph
    - Python
