...
* `iter()` 메서드는 에이전트의 실행 과정을 단계별로 반복할 수 있게 해주는 반복자(iterator)를 생성한다.
    * 실행 별 과정을 좀 더 세부적으로 관잘하고 처리할 수 있는 기능
* 중간 과정에서 사용자의 입력을 받아 계속 진행할지 묻는 기능을 제공한다. 이를 `Human-in-the-loop` 라고 한다.
...
**함수 설명**
`iter()` 는 에이전트가 최종 출력에 도달하기까지 거치는 단계들을 순차적으로 접근할 수 있는 `AgentExecutorIterator` 객체를 반환한다.

**주요 기능**
- **단계별 실행 접근**: 에이전트의 실행 과정을 단계별로 살펴볼 수 있다.
...
`"114.5 + 121.2 + 34.2 + 110.1"` 의 덧셈 계산을 수행하기 위해서는 단계별로 계산이 수행된다.
