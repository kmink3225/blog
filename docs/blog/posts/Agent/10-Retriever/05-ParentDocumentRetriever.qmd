...
* Child Document
    * Document = Child Document = Sub Document = Split = Split Document = Chunk 모두 같은 말
* Parent Document: Child Document의 원본
* Embeding 또는 특정 task 또는 Project의 문제를 해결하기 위한 Retriever의 아키텍쳐를 구성할 때 Child Document 뿐만 아니라 Parent Document역시 필요한 경우가 있다.
* 뿐만 아니라, 단일 Retriever를 구성하여 검색된 Child Document를 바로 LLM에 입력하는 것 보다 검색된 Child Document가 포함된 앞뒤 문맥을 Parent Document로 부터 추출하여 이를 다른 LLM이 실시간으로 요약한 결과를 RAG의 LLM에 입력하는게 할루시네이션을 줄이거나 답변의 질을 높일 수 있다. 
* Parent Document Retriever는 Parent Document, 즉 원본문서가 필요할 때 사용할 수 있는 Retriever다
...
* 위의 예시에서는 `.txt`로 로드가 되어 있기 때문에 파일 내에 있는 전체 내용이 parent document가 된다.
    * 만약 `len(docs)`를 출력하면 parent document의 수 `1`이 출력된다.
* 단, `.pdf`의 경우 업로드시 패이지 단위로 문서가 업로드 되어 parent document는 pdf의 page가 된다.
    * 만약 `len(docs)`를 출력하면 parent document의 수 `페이지수`가 출력된다.
...
* 1개가 출력됐는데 parent document가 1개 있다는 의미, 하지만 chunk는 여러개.

이제 벡터 스토어 검색 기능을 호출해 보겠습니다.

우리가 작은 청크(chunk)들을 저장하고 있기 때문에, 검색 결과로 작은 청크들이 반환되는 것을 확인할 수 있을 것입니다.

`vectorstore` 객체의 `similarity_search` 메서드를 사용하여 `Word2Vec` 와 관련성이 높은 문서를 찾아내는 유사도 검색을 수행합니다.
...
`sub_docs[0].page_content` 즉, chunk 를 출력합니다.
...
Parent Document (`retrieved_docs[0]`)의 일부 내용을 출력합니다.
...
* 1000개 단위로 쪼개진 상위 chunk가 `7`개 라는 의미
