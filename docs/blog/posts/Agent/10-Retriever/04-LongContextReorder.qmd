...
subtitle: "Lost in the Middle 현상, Attention 희석, 그리고 RAG 성능 최적화"
description: |
  LLM이 긴 문맥의 중간 정보를 간과하는 'Lost in the Middle' 현상의 원인을 분석한다. Context window 크기와의 관계, Attention 희석 메커니즘, Long context 모델의 이중 취약성을 다루고, LangChain의 LongContextReorder와 시맨틱 태그 기반 프롬프트 구조화를 통한 대응 전략을 정리한다.  
categories:  
  - AI  
  - RAG  
  - LangChain  
...
## Context Window 크기와 Lost in the Middle의 관계

### Context Window가 커지면 더 심해지는가?

**Yes.** Lost in the Middle은 context window 크기와 무관하게 발생하지만, context가 길어질수록 U자형 골이 **깊고 넓어진다.**

| Context 크기 | Lost in the Middle 심각도 | 이유 |
|-------------|--------------------------|------|
| 4K 토큰 | 발생하지만 경미 | 전체 내용이 짧아서 "중간"이 상대적으로 가까움 |
| 32K 토큰 | 뚜렷하게 발생 | 중간 영역이 넓어짐 |
| 128K+ 토큰 | 가장 심각 | 중간 영역이 매우 넓고, attention이 희석됨 |

4K에서 "중간"이라고 해봐야 2K 지점이지만, 128K에서는 "중간"이 30K~90K 구간에 해당한다. **중간 영역의 절대적 크기가 커지므로 정보 손실 구간도 비례하여 확대**된다.

### Attention 희석 (Attention Dilution)

Lost in the Middle의 근본 원인 중 하나는 Transformer의 softmax attention이 확률 분포라는 점에서 비롯된다.

```python
# 짧은 시퀀스 (1K 토큰)
attention_weight = softmax(Q @ K.T / sqrt(d))
# 각 토큰에 평균 1/1000 = 0.001 할당

# 긴 시퀀스 (128K 토큰)
attention_weight = softmax(Q @ K.T / sqrt(d))
# 각 토큰에 평균 1/128000 = 0.0000078 할당
```

**softmax가 더 많은 토큰에 확률을 분배해야 하므로, 개별 토큰에 대한 attention이 희석**된다. 이것은 수학적으로 불가피한 현상이다.

### Positional Bias vs Attention 희석

두 현상은 관련되어 있지만 메커니즘이 다르다.

| 측면 | Positional Bias (Lost in the Middle) | Attention 희석 |
|------|--------------------------------------|---------------|
| 원인 | 학습 데이터에서 형성된 위치 편향 | softmax 확률 분배의 수학적 한계 |
| 발생 위치 | 중간 구간에 집중 | 전체적으로 균일하게 발생 |
| 모델 크기 영향 | 큰 모델도 동일 패턴 | 큰 모델에서 약간 완화 |
| 해결 가능성 | 학습 방식으로 일부 완화 가능 | 아키텍처 한계 (근본적) |

::: {.callout-important}
**이중 취약성**: Long context 모델에서는 두 현상이 동시에 작용한다.

1. attention 희석으로 중간 토큰에 대한 attention이 약해지고
2. positional bias로 중간 토큰이 추가로 무시됨
3. 두 효과가 곱해져서 중간 구간의 정보 손실이 가속됨
:::

### Long Context 모델의 개선 노력과 한계

최근 모델들이 시도하는 대응 방식이다.

#### RoPE 스케일링 (Position Embedding 개선)

기존에는 학습된 위치 범위(예: 4K)를 넘기면 성능이 급락했다. YaRN, NTK-aware 스케일링 등으로 위치 인코딩을 확장하여 긴 시퀀스 처리 자체는 가능해졌지만, **Lost in the Middle 패턴은 여전히 관찰**된다.

#### 학습 데이터 구성

정답이 중간에 있는 학습 데이터를 의도적으로 늘려 positional bias를 완화한다. Claude 3.5, GPT-4 Turbo 등 최신 모델에서 부분적으로 개선되었지만 완전히 해결되지는 않았다.

#### Sliding Window Attention

Mistral 등이 사용하는 방식으로, 전체 시퀀스에 대한 full attention 대신 window 단위로 attention을 계산한다. attention 희석은 줄이지만, **window 밖의 정보에 접근하기 어렵다는 새로운 trade-off**가 생긴다.

### 프롬프트 구조화로 Lost in the Middle 완화

LongContextReorder가 RAG의 검색 문서 배치를 다룬다면, **시스템 프롬프트 내부에서도 동일한 원리를 적용**할 수 있다.

#### 배치 전략

```
[최우선 규칙]           <- 앞쪽: Primacy bias 활용
[중요도 낮은 규칙]       <- 중간: 놓쳐도 치명적이지 않은 내용
[Few-shot 예시]         <- 중간~뒤쪽
[Context/문서]          <- 뒤쪽
[질문 + 출력 지시]       <- 맨 뒤: Recency bias 활용
```

#### 시맨틱 XML 태그의 앵커 효과

```xml
<!-- XML 태그가 attention의 "앵커" 역할 -->
<boundaries>가장 중요한 제약 조건</boundaries>

<domain-rules>도메인 규칙 (중간에 위치해도 태그로 강조)</domain-rules>

<output-format>출력 형식 지시</output-format>
```

시맨틱 XML 태그는 positional bias를 완화하는 실용적 수단이다. 태그 자체가 attention을 끌어모으는 앵커 역할을 하여, 중간에 위치한 내용도 놓치지 않게 돕는다. 비구조화된 텍스트 나열에서는 중간 규칙이 무시되기 쉽지만, `<boundaries>`나 `<fix-...>` 같은 태그로 감싸면 LLM이 섹션 경계를 인식하여 중간 내용도 잘 참조한다.

::: {.callout-tip}
RAG에서는 **LongContextReorder**로 검색 문서를 재배치하고, 시스템 프롬프트에서는 **시맨틱 태그 + 배치 전략**으로 중요 규칙의 위치를 관리하는 것이 Lost in the Middle에 대한 이중 방어선이다.
:::
