...
subtitle: 작은 정책 모델로 방향성 힌트를 생성하여 LLM의 출력을 원하는 방향으로 유도하는 기법
description: |
  Directional Stimulus Prompting의 정의부터 실전 구현까지 체계적으로 설명한다.
  Li et al. (2023) 연구를 바탕으로, 작은 정책 LM이 생성한 방향성 힌트(키워드, 핵심 구절)를
  프롬프트에 포함하여 LLM의 출력 품질을 향상시키는 방법을 분석한다.
categories:
  - Prompt Engineering
  - LLM
  - AI
