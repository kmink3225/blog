...
단위원 $\{\mathbf{x} : \|\mathbf{x}\| = 1\}$ 의 이미지 $\mathbf{A}\{\mathbf{x}\}$ 는 **타원**(또는 고차원의 초타원체)이다. 이 타원의:
...
**1. $\mathbf{v}_1, \ldots, \mathbf{v}_r$ 은 행 공간의 기저이다.**
...
**2. $\mathbf{v}_{r+1}, \ldots, \mathbf{v}_n$ 은 영공간의 기저이다.**
$\sigma_i = 0$ 이면 $\mathbf{A}^\top\mathbf{A}\mathbf{v}_i = 0 \cdot \mathbf{v}_i = \mathbf{0}$, 따라서 $\|\mathbf{A}\mathbf{v}_i\|^2 = \mathbf{v}_i^\top\mathbf{A}^\top\mathbf{A}\mathbf{v}_i = 0$, 즉 $\mathbf{A}\mathbf{v}_i = \mathbf{0}$. 이 벡터들이 영공간에 들어간다.

**3. $\mathbf{u}_1, \ldots, \mathbf{u}_r$ 은 열 공간의 기저이다.**
$\mathbf{u}_i = \mathbf{A}\mathbf{v}_i / \sigma_i$ — 즉 $\mathbf{u}_i$ 는 $\mathbf{A}$ 의 열들의 선형 결합이므로 자동으로 열 공간 안에 있다. $r$ 개가 직교하므로 독립이고, 열 공간의 차원이 $r$ 이므로 기저가 된다.

**4. $\mathbf{u}_{r+1}, \ldots, \mathbf{u}_m$ 은 좌영공간의 기저이다.**
$\mathbf{U}$ 의 모든 열이 직교하므로 $\mathbf{u}_i$ ($i > r$) 는 열 공간과 직교한다. 열 공간의 직교 여공간이 좌영공간이므로 이 벡터들이 좌영공간을 채운다.

하나의 식 $\mathbf{A}\mathbf{v}_i = \sigma_i \mathbf{u}_i$ 가 네 부분공간을 동시에 조립한다. 이것이 SVD를 "기본정리의 완성"이라 부르는 이유다.

---

## 변분적 정의 — 최대화 문제로서의 SVD

SVD를 이해하는 또 하나의 시각이 있다. 특이값은 **최대화 문제의 해** 로도 정의할 수 있다.

$$\sigma_1 = \max_{\|\mathbf{x}\| = 1} \|\mathbf{A}\mathbf{x}\|$$

즉 $\sigma_1$ 은 "단위 벡터에 $\mathbf{A}$ 를 곱했을 때 가장 크게 늘어난 길이" 다. 최대를 달성하는 $\mathbf{x}$ 가 바로 $\mathbf{v}_1$ 이고, 그때의 이미지가 $\sigma_1 \mathbf{u}_1$ 이다.

두 번째 특이값은 $\mathbf{v}_1$ 을 **제외한 방향** 중에서 최대값이다:
...
이 관점에서 **PCA**가 자연스럽게 나온다: "데이터에서 분산이 가장 큰 방향" = "$\|\mathbf{X}\mathbf{x}\|$ 를 최대화하는 방향" = "$\mathbf{X}$ 의 첫 번째 right singular vector $\mathbf{v}_1$". PCA는 SVD를 최적화 문제로 본 것일 뿐이다.
...
- $\mathbf{U}\mathbf{\Sigma}$ 의 열: **주성분 점수** (principal component scores)
- $k$ 개 주성분으로 차원 축소: $\mathbf{X}_k = \mathbf{U}_k \mathbf{\Sigma}_k \mathbf{V}_k^\top$

::: {.callout-warning}
## 실무에서 SVD 경로가 더 안정적이다

수학적으로 두 경로는 동일하지만, 수치적으로는 다르다. $\mathbf{X}^\top\mathbf{X}$ 를 직접 만들면 조건수가 $\kappa(\mathbf{X})^2$ 로 **제곱** 되어 작은 특이값 정보가 날아간다. 실무 PCA 구현(`sklearn.decomposition.PCA`)이 내부적으로 공분산 행렬을 만들지 않고 SVD를 직접 호출하는 이유다.
:::
...
### $\kappa^2$ 효과 — 왜 $\mathbf{A}^\top\mathbf{A}$ 를 피해야 하는가
...
따라서 **LAPACK의 svd 함수**는 $\mathbf{A}^\top \mathbf{A}$ 를 직접 계산하지 않고, 대신 $\mathbf{A}$ 자체에 이분할(bidiagonalization) 과정을 적용하는 **Golub-Reinsch 알고리즘** 을 사용한다. NumPy/R의 `svd()` 함수도 모두 이 방식을 사용한다.
...
4. **고유값 분해의 일반화**: 정방 행렬이어야만 했던 고유값 분해를 **모든 행렬** 로 확장하면서, $\mathbf{U}$ 와 $\mathbf{V}$ 가 **항상 직교** 라는 더 강한 성질까지 보장한다.
