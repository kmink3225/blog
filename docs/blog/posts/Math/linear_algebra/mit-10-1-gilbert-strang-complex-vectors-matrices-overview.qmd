---
title: "복소 벡터와 행렬 개요 (Complex Vectors and Matrices Overview)"
subtitle: "왜 실수만으로는 부족한가 — Hermitian·Unitary·Fourier의 지도"
description: |
  회전 행렬은 실 벡터로는 불변 방향을 갖지 않지만, 복소 벡터로 확장하면
  $e^{i\theta}$ 와 그 켤레쌍이 고유값으로 등장한다. 실행렬도 복소 고유값을
  갖고, 많은 선형대수 도구(푸리에 변환, 양자역학, 신호처리)는 복소 공간에서만
  정확히 작동한다. 이 포스트는 Strang Ch.10 을 뼈대로 복소수 기본 →
  켤레전치(Hermitian) → 복소 내적 → Hermitian/Unitary 행렬의 스펙트럼 정리 →
  푸리에 행렬과 FFT → ML·DL 응용까지를 한 편에 개관한다. 각 섹션은 실수
  세계의 대응 개념과 짝지어 "무엇이 바뀌고 무엇이 보존되는가"를 명시한다.
categories:
  - Mathematics
  - Machine Learning
  - Deep Learning
...
이 한 사례가 선형대수 전체에서 **복소수 확장이 선택이 아니라 필수**임을 보여 준다.
...
$i$ 는 방정식 $x^2 = -1$ 의 해로 정의된다. 실수로는 해를 가질 수 없으므로 **수 체계의 확장**이며, 그 확장은 $i^2 = -1$ 이라는 단 하나의 규칙으로 충분하다.
...
- **합**: $(a + bi) + (c + di) = (a+c) + (b+d)i$
- **곱**: $(a + bi)(c + di) = (ac - bd) + (ad + bc)i$ — $i^2 = -1$ 로부터 유도
...
복소수 $z = a + bi$ 는 평면 $\mathbb{R}^2$ 의 점 $(a, b)$ 에 대응한다. 합은 벡터 덧셈과 동일하다. 차이는 **곱셈 연산이 복소평면에 존재**한다는 점이다 — 복소수 곱은 평면에서 "크기 곱 + 각도 합"이다.
...
곱셈 $\times e^{i\theta}$ 는 복소평면에서 **원점 기준 $\theta$ 회전**이다. 이 관점에서
...
**실 행렬의 복소 고유쌍**: $A \mathbf{v} = \lambda \mathbf{v}$ 이고 $A$ 가 실이면, 양변에 켤레를 취하면 $A\bar{\mathbf{v}} = \bar{\lambda}\bar{\mathbf{v}}$. 즉 **복소 고유값은 반드시 켤레쌍 $(\lambda, \bar{\lambda})$ 으로 등장**한다. 그래서 회전 행렬이 $e^{i\theta}, e^{-i\theta}$ 를 함께 갖는다.
...
로 정의된다. 이 내적은 **첫 인자에 대해 켤레선형, 둘째에 대해 선형**이다 (물리학 규약).
:::

주의: $\mathbf{u}^H \mathbf{v} \neq \mathbf{v}^H \mathbf{u}$. 대신 $\mathbf{v}^H \mathbf{u} = \overline{\mathbf{u}^H \mathbf{v}}$ 이다. 복소 내적은 **비가환**이며, 이는 복소 세계를 실 세계와 구분 짓는 본질적 성질이다.
...
## Hermitian 행렬: 실 대칭의 복소 일반화

### 정의

$A = A^H$ 인 행렬을 **Hermitian 행렬**이라 한다. 실 행렬이면 $A^H = A^\top$ 이므로 "실수 대칭 = 실수 Hermitian"이고, Hermitian은 대칭의 진짜 복소 일반화다.

성분 조건: $a_{ij} = \overline{a_{ji}}$. 따라서 **대각 성분은 실수**이고, 비대각 쌍은 켤레 관계다.
...
증명: $\mathbf{z}^H A \mathbf{z}$ 는 $1\times 1$ 스칼라이므로 $(\mathbf{z}^H A \mathbf{z})^H = \mathbf{z}^H A^H \mathbf{z} = \mathbf{z}^H A \mathbf{z}$. 자기 자신과 켤레가 같으므로 실수. $\square$
...
## Unitary 행렬: 직교 행렬의 복소 일반화

### 정의

$U^H U = I$ 인 정방 행렬을 **unitary 행렬**이라 한다. 등가적으로 $U^{-1} = U^H$.
...
좌우가 1:1 대응한다는 것이 복소 선형대수의 큰 그림이다. **$^\top$ 을 $^H$ 로만 바꾸면** 실수 세계의 정리가 그대로 복소 세계에서 성립한다.
...
Strang §10.3 의 주인공은 **$n$ 차 푸리에 행렬**이다. $\omega = e^{2\pi i/n}$ ("1의 $n$ 제곱근") 에 대해
...
$F_n / \sqrt{n}$ 은 **unitary** 행렬이다. 즉 $F_n^H F_n = n I$.

이 행렬은 "시간 영역 → 주파수 영역" 변환을 실현한다. 벡터 $\mathbf{c}$ 에 $F_n$ 을 곱하면 $\mathbf{c}$ 를 여러 주파수의 진동으로 **분해한 계수**가 나온다.

### FFT가 왜 중요한가

직접 곱셈 $F_n \mathbf{c}$ 는 $O(n^2)$ 연산이다. Cooley-Tukey (1965)의 **Fast Fourier Transform (FFT)** 은 $F_n$ 의 재귀적 구조를 이용해
...
FFT의 재귀 원리는 "짝수 인덱스와 홀수 인덱스를 분리하면 $F_n$ 이 $F_{n/2}$ 둘로 쪼개진다"는 것이다. Strang §10.3 는 이 분해
...
- **Neural Operator (FNO)**: PDE 해를 학습할 때 $F$ 와 $F^{-1}$ 사이에 학습 가능한 대각 행렬을 두어 *주파수 도메인에서* 선형 변환을 학습한다. 메시·격자 독립성과 $O(n\log n)$ 복잡도가 매력.
...
- [§10.1 복소수 기본 (placeholder)](./mit-10-2-gilbert-strang-complex-numbers.qmd) — $i^2 = -1$, 극형식, Euler 공식
- [§10.2 Hermitian과 Unitary 행렬 (placeholder)](./mit-10-3-gilbert-strang-hermitian-unitary.qmd) — 스펙트럼 정리의 복소판
- [§10.3 Fast Fourier Transform (placeholder)](./mit-10-4-gilbert-strang-fft.qmd) — $F_n$ 의 재귀 분해, $O(n \log n)$
...
- [고유값과 고유벡터](./mit-06-1-gilbert-strang-eigenvalue-overview.qmd) — 실 고유값이 부족한 상황 동기
- [대칭 행렬과 스펙트럼 분해](./mit-06-4-gilbert-strang-symmetric-matrices.qmd) — 실 대칭의 스펙트럼 정리
- [직교 기저와 Gram-Schmidt](./mit-04-5-gilbert-strang-orthogonal-bases-gram-schmidt.qmd) — 직교의 개념
- [SVD](./mit-06-7-gilbert-strang-svd.qmd) — $\sigma_i$ 는 $A^H A$ 고유값의 제곱근
...
- [Attention과 Fourier 기반 시계열 모델](../../Deep_Learning/index.qmd) — FFT, FNO (placeholder)
- [Spectral GNN과 그래프 라플라시안](../../Deep_Learning/index.qmd) — Hermitian 라플라시안 (placeholder)
- [신호 전처리와 주파수 분석](../../Statistics/index.qmd) — 시계열의 푸리에 분해 (placeholder)
