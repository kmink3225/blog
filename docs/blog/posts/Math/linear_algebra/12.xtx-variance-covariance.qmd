...
여기서 $\mathbf{1}$ 은 모든 원소가 1인 $n$ 차원 벡터다. $H$ 는 대칭이고 멱등( $H^2 = H$ )이다.
...
OLS 추정량 $\hat{\beta} = (X^\top X)^{-1} X^\top y$ 에서 $X^\top X$ 는 세 가지 역할을 한다:
...
- [Basic Vector(2) — Norm과 Dot Product](./02.norm_dot-product.qmd)
- [Basic Matrix(2) — Matrix Multiplication](./05.matrix_multiplication.qmd)
