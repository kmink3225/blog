...
**LAST_VALUE 함정**: 기본 프레임은 `ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW` (현재 행까지). 그래서 LAST_VALUE는 "현재 행까지 중 마지막" = 현재 행 자신을 반환한다. 진짜 마지막을 원하면 **프레임을 UNBOUNDED FOLLOWING까지 명시**해야 한다.

## WINDOW 절 — 명명 윈도우 (같은 윈도우 재사용)
