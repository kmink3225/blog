...
subtitle: docker_compose.yaml 해석, Postgres Container 올리기, Connection, Hook, bulk_load(), Custom Hook, More Providers, Connection Type 
description: |
...
* conn session (con object)에서 sql을 이용한 구체적인 query 내용은 두 번째 with문에서 기술
  ```markdown
   with closing(conn.cursor()) as cursor:
                ...
                conn.commit()
  ```
    * cursor: client(python로직을 호출하는 worker container)와 DB서버(container) 사이의 session(client와 DB 서버와의 connection 역할)안에서 client에서 query를 날리고 DB서버로부터 결과를 가져와주는 object. cursor에서 sql 수행. 그러므로 cursor (=conn.cursor())가 없으면 query 수행을 못함
