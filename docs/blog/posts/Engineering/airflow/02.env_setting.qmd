...
* docker 설치 확인 
    * `sudo docker run hello-world` 하면 다음과 같은 메세지 떠야함  
    ![docker 설치 확인](../../../../../images/airflow/check_docker_installation.PNG)
...
* 두 번째 터미널 열고 `sudo docker ps` 실행하여 container list 상태 확인. 총 6개 올라와야 정상
    ...
    * airflow-postgres-1
    * airflow-redis-1 : for queue service
* 웹 브라우저 창에 `localhost:8080` 입력하여 airflow service창에 접속
    ...
    * Airflow Webserver (or Web Browser) Port 변경
        * Airflow의 default port 는 `8080`으로 설정되어 있는데 만약 port를 변경하고 싶으면 `docker-compose.yaml`을 수정해야한다. 다음은 `docker-compose.yaml`의 airflow webserver 부분을 복붙한 것이다.
        <변경전 with comments>
        ```markdown
          airflow-webserver:
            <<: *airflow-common
          ...
          airflow-webserver:
            ...
            networks:
              network_custom:
                ipv4_address: 172.28.0.6
        ```
        위 처럼 변경하면 되지만 나는 그냥 기본 port인 `8080`을 사용한다.
