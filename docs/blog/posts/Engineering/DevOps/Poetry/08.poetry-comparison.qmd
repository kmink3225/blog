---
title: "Poetry vs pip vs Conda vs Pipenv vs PDM"
subtitle: "Python 패키지 관리 도구 비교와 선택 기준"
description: |
  Python 생태계의 주요 패키지 관리 도구들을 비교한다.
  pip, Conda, Pipenv, Poetry, PDM의 특징과 장단점,
...
| 기능 | pip | Conda | Pipenv | Poetry | PDM |
|------|-----|-------|--------|--------|-----|
| 의존성 설치 | O | O | O | O | O |
| Lock 파일 | X | X | O | O | O |
| 가상환경 관리 | X (별도) | O | O | O | O |
| 의존성 해결 | 제한적 | O | O (느림) | O (빠름) | O (빠름) |
| 빌드/배포 | X | X | X | O | O |
| pyproject.toml | X | X | X | O | O |
| 의존성 그룹 | X | X | dev만 | O | O |
| 비-Python 패키지 | X | O | X | X | X |
| PEP 621 지원 | - | - | - | 2.x부터 | O |
| 설치 용이성 | 기본 내장 | 별도 설치 | pip 설치 | 별도 설치 | pip 설치 |
