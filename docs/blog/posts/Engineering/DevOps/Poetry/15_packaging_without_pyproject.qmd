---
title: "pyproject.toml 없는 repo 패키지화 — setuptools 설정 작성과 PR 반영"
date: 2026-03-04
categories: [python, setuptools, packaging, pyproject]
---

## 개요

외부 repo에 `pyproject.toml`이 없으면 Poetry는 설치를 거부합니다.
...
이 repo의 코드를 직접 수정할 권한이 없거나(archive 상태),
수정 후 PR을 통해 반영해야 하는 상황에서
어떻게 `pyproject.toml`을 설계하고 추가하는지 다룹니다.

---

## 1. 대상 repo 구조 파악

먼저 repo를 clone해서 실제 구조를 확인합니다.
...
`source/` 디렉토리에 실제 Python 코드가 있습니다.
...
::: {.callout-note}
일반적인 Python 패키지 구조라면 `src/패키지명/` 또는 `패키지명/`이 루트에 있어야 합니다.
이 repo는 `source/`라는 비표준 디렉토리명을 사용하고 있습니다.
이 부분을 `pyproject.toml`에서 매핑해야 합니다.
:::
...
패키지 이름과 공개 API를 파악하기 위해 `__init__.py`를 확인합니다.
...
`import data_standardization` 시 이 클래스들이 노출됩니다.

---

## 3. `pyproject.toml` 설계 원칙
...
## 5. archive 상태 repo에 pyproject.toml 추가하기

repo가 archive 상태이면 직접 push가 불가합니다.
...
PR이 승인되기를 기다리는 동안 개발을 멈출 필요가 없습니다.
지금 가지고 있는 local clone에 직접 `pyproject.toml`을 추가하고
**local path 의존성**으로 임시 설치합니다.
...
PR이 merge되어 remote에 `pyproject.toml`이 반영되면
local path를 git URL로 교체합니다.
...
local path에서 remote commit(`f3354b9`)으로 전환 완료.

---

## 8. git pull 시 충돌 주의

local clone에 untracked 파일(`pyproject.toml`)이 있는 상태에서
remote에 같은 파일이 추가되면 `git pull`이 실패합니다.
...
이후 remote의 `pyproject.toml`이 자동으로 내려옵니다.
...
| archive repo 우회 | unarchive → PR → merge 흐름 |
| 개발 continuity | PR 승인 전 local path로 임시 설치 |
| 전환 방법 | `file://` → `git+ssh://` 교체 후 `poetry lock` |
| git pull 충돌 | untracked 파일 제거 후 pull |

다음 블로그에서는 **local path에서 git URL로 전환하는 전체 흐름**과
`poetry.lock`에서 확인하는 방법을 다룹니다.
