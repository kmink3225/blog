...
`fingerprint`와 `randomart`는 따로 저장할 필요 없다. 언제든 재확인할 수 있다:
...
멀티 계정이 아닌 단일 계정이라면 `Host github.com`으로 그대로 사용하면 된다.
...
처음 접속 시 known_hosts 추가 여부를 묻는다. `yes`를 입력한다:
