---
title: "Python 빌드 시스템(Poetry, setuptools)과 pyproject.toml 이해"
subtitle: Poetry, setuptools, PEP 517/518, pyproject.toml, 패키지 구조 일반화
description: |
    Poetry와 setuptools 등 다양한 빌드 시스템이 pyproject.toml로 통합 관리되는 원리, PEP 517/518 표준의 역할, wheel의 의미, 패키지 이름/임포트 이름 분리 등 실무 사례와 일반 원칙을 함께 정리한다.
categories:
    - Engineering
    - Python
    - DevOps
...
* Poetry로 관리되는 프로젝트에서 setuptools 기반의 외부 패키지를 설치하려 할 때, "빌드 백엔드가 다르면 설치가 안 되는 것 아닌가?"라는 의문이 생긴다.
* AI와 구글링한 결과 **가능하다.** 그 이유는 PEP 517/518 표준이 있기 때문이다.
