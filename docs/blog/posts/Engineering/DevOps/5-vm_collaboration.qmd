...
## VS Code Remote SSH를 활용한 현실적인 타협점: 

* Remote SSH는 결국 **VM 코드를 직접 작업**하는 것이다. 
...
* 운영 경로는 `git pull`만 하는 읽기 전용으로 유지한다.  
...
* Azure VM의 네트워크 보안 그룹(NSG)에서 해당 포트들이 인바운드 허용되어 있어야 외부에서 접근할 수 있다.
