...
# 이후 수정 작업을 더 한 다음 직접 커밋
git add .
git commit -m "cherry-pick 적용 및 추가 수정"
```

여러 커밋의 변경사항을 하나의 커밋으로 합칠 때 유용하다.
...
```bash
# 1. 충돌 발생 시 git status로 충돌 파일 확인
git status
...
# ghi9012 feat: Add refund feature    ← 아직 릴리스 불가
# def5678 feat: Add payment history   ← 릴리스 가능
# abc1234 feat: Add payment gateway   ← 릴리스 가능
