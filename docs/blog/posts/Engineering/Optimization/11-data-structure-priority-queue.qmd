...
description: |
  우선순위 큐는 힙 구조를 기반으로 삽입과 삭제 모두 $O(\log N)$ 에 동작하며 최단 경로 알고리즘 등에 활용된다.
categories:
  - Engineering
  - Data Structure
author: Kwangmin Kim
...
`heapq`는 최소 힙만 제공하므로, 최대 힙이 필요할 때는 값에 음수 부호를 붙여 삽입한다. 꺼낼 때 다시 음수를 취하면 원래 최댓값을 얻는다.
