...
## 주요 브랜치 종류 및 기본 전략 (Git Flow 예시)

일반적으로 사용되는 브랜치 종류와 간단한 브랜칭 전략(예: Git Flow)은 다음과 같다.

*   **`main` (또는 `master`)**: 배포 가능한 안정적인 버전의 코드를 관리하는 주 브랜치.
*   **`develop`**: 다음 릴리스 버전을 개발하는 통합 브랜치. `feature` 브랜치들이 이곳으로 병합된다.
*   **`feature/*`**: 특정 기능 개발을 위한 브랜치. `develop` 브랜치에서 분기하여 개발 완료 후 다시 `develop`으로 병합된다. (예: `feature/user-authentication`)
*   **`release/*`**: 새로운 버전 배포를 준비하기 위한 브랜치. `develop` 브랜치에서 분기하며, 버그 수정 및 최종 테스트 후 `main`과 `develop` 브랜치 양쪽으로 병합된다. (예: `release/v1.2.0`)
*   **`hotfix/*`**: 배포된 `main` 브랜치에서 발생한 긴급 버그를 수정하기 위한 브랜치. `main`에서 분기하여 수정 후 `main`과 `develop` 양쪽으로 병합된다. (예: `hotfix/critical-bug-123`)

이러한 전략은 팀의 규모나 프로젝트 성격에 따라 유연하게 조정될 수 있다.

## 개인 작업용 브랜치 활용 전략

팀 프로젝트에서 개인적인 실험이나 백업, 또는 팀 리뷰와 분리된 작업을 관리하기 위해 별도의 개인 브랜치를 사용하는 전략도 유용하다.

### 예시: 두 가지 목적의 브랜치 운영

1.  **`feature/token-processor-improvements` (팀 리뷰 및 운영 코드용)**
    *   팀원들과 공유하고 리뷰 받아야 하는 운영 코드(예: `1_data_preparation/` ~ `8_token_analyzer/` 폴더)만 포함한다.
    *   개인 작업 폴더(예: `kmkim/`)는 `.gitignore`를 통해 제외하여 팀원들이 리뷰할 때 불필요한 파일을 보지 않도록 한다.
2.  **`personal/kmkim-workspace` (개인 백업 및 실험용)**
    *   `kmkim/` 폴더와 같이 개인적인 파일이나 실험적인 코드를 포함하여 GitHub에 백업한다.
    *   다른 작업 환경에서도 개인 작업을 이어갈 수 있도록 한다.
    *   팀 리뷰와는 독립적으로 관리한다.
