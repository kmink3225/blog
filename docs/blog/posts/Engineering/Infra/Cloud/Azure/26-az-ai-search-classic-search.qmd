...
- NLP Skills: 핵심 구문 추출, 개체명 인식, 언어 감지, 감정 분석, PIIDetection 등으로 다양한 언어와 처리 지연 특성을 가짐.  
- Vision Skills: OCR(문서 중심)과 이미지 분석(장면 태깅)을 제공하며, OCR은 페이지당 수초 수준의 처리 시간이 소요될 수 있음.  
- Custom Skills: Azure Functions 또는 외부 Web API로 커스텀 전처리·분류·임베딩을 통합할 수 있으며 배치 크기와 병렬도 조정으로 처리량을 튜닝한다.  

실무 팁: AI enrichment는 비용·지연을 유발하므로 우선순위가 높은 변환만 파이프라인에 포함하고 배치 처리로 비용을 제어한다.  

## Integrated Vectorization (요약)  

Integrated Vectorization은 인덱싱 파이프라인 내에서 임베딩을 자동 생성해 일관성과 운영 편의성을 제공한다.  

- 장점: 별도 임베딩 파이프라인 불필요, 인덱싱·쿼리 간 모델 불일치 위험 감소.  
- 단점: 쿼리 지연 소폭 증가(쿼리-time vectorization), 모델 비용 및 rate-limit 영향 고려 필요.  

지원 vectorizer 유형: Azure OpenAI 임베딩, Vision 멀티모달, 또는 커스텀 Web API 연결.  

권장: 프로덕션에서는 사전생성(pre-vectorized)와 통합 vectorization 중 운영·비용·성능 요구에 따라 선택한다.  
