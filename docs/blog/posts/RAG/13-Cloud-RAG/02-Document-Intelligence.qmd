...
Azure Document Intelligence(이전 Form Recognizer)는 AI 기반 문서 이해 서비스로, 스캔 문서나 이미지에서 텍스트, 표, 구조를 추출한다. RAG 시스템에서 PDF, 이미지 등의 비정형 문서를 구조화된 텍스트로 변환하는 핵심 역할을 한다.
...
- **레이아웃 분석**: 제목, 단락, 표, 리스트 구분
- **다국어 지원**: 100개 이상 언어 (한국어, 영어, 중국어, 일본어 등)
- **사전 훈련 모델**: 영수증, 명함, 청구서 등 특화 모델 제공

## Document Intelligence 모델

Azure Document Intelligence는 문서 유형에 따라 다양한 사전 훈련 모델을 제공한다:

### 1. Layout API (범용 레이아웃 분석)
...
- "Document Intelligence" 검색 및 선택

**2. 기본 설정:**
- **구독**: 사용할 구독 선택
- **리소스 그룹**: `rg-rag-prod`
- **지역**: Korea Central (한국어 처리 최적화)
- **이름**: `doc-intel-rag-prod`
- **가격 책정 계층**:
  - **Free F0**: 500페이지/월 무료 (개발/테스트)
  - **Standard S0**: 종량제 (프로덕션)
...
단순 텍스트 추출만 필요할 경우 Read API가 더 빠르고 저렴하다.
...
**Layout vs Read 비교:**
| 항목 | Layout API | Read API |
|------|-----------|----------|
| **가격** | $0.01/페이지 | $0.0015/페이지 |
| **속도** | 느림 | 빠름 |
| **구조 정보** | ✅ (제목, 표, 단락) | ❌ |
| **RAG 권장** | 복잡한 문서 | 간단한 텍스트 문서 |
...
**1. InvalidRequest: 파일 크기 초과**
```
오류: 파일이 너무 큼 (최대 500MB)
해결: 파일을 분할하거나 압축
```

**2. InvalidImage: 이미지 해상도 부족**
```python
