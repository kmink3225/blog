...
Azure OpenAI Embeddings는 텍스트를 고차원 벡터로 변환하는 서비스다. RAG 시스템에서 문서를 숫자 벡터로 표현하여 의미 기반 검색을 가능하게 한다.
...
### 2. text-embedding-3-small (권장)
**출시**: 2024년 1월

**사양:**
- 차원: 512 또는 1536 (선택 가능)
- 최대 토큰: 8,191
- 가격: $0.02 / 1M 토큰 (ada-002 대비 **5배 저렴**)
...
**3. 네트워킹:**
- 모든 네트워크 (개발) 또는 선택된 네트워크 (프로덕션)

**4. 검토 + 만들기** → 생성 완료

### 모델 배포

Azure OpenAI에서는 리소스 생성 후 **모델을 배포**해야 한다.

**1. Azure OpenAI Studio 접속:**
- 생성된 리소스 → "Azure OpenAI Studio로 이동"

**2. 배포 만들기:**
- "배포" 메뉴 → "+ 배포 만들기"
- **모델 선택**: text-embedding-3-small
- **배포 이름**: `text-embedding-3-small` (또는 원하는 이름)
- **모델 버전**: 최신 버전
- **TPM (분당 토큰)**: 120K (기본값)
